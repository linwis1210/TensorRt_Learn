...
__global__ void pre_process(float *intensity, float *resc, float *mean, float *std,const int row, const int clos) {
  ...
  if((tx<row-3) && (ty<clos-3)){
        int x = tx % 4;
        int y = ty % 4;
        resc[(tx*(clos-3)+ty)*16] = (intensity[(tx+(4-x)%4)*clos+ty+(4-y)%4] - mean[0]) / std[0];
        resc[(tx*(clos-3)+ty)*16+1] = (intensity[(tx+(4-x)%4)*clos+ty+(5-y)%4] - mean[1]) / std[1];
        resc[(tx*(clos-3)+ty)*16+2] = (intensity[(tx+(4-x)%4)*clos+ty+(6-y)%4] - mean[2]) / std[2];
        resc[(tx*(clos-3)+ty)*16+3] = (intensity[(tx+(4-x)%4)*clos+ty+3-y] - mean[0]) / std[0];
        
        resc[(tx*(clos-3)+ty)*16+4] = (intensity[(tx+(5-x)%4)*clos+ty+(4-y)%4] - mean[3]) / std[3];
        resc[(tx*(clos-3)+ty)*16+5] = (intensity[(tx+(5-x)%4)*clos+ty+(5-y)%4] - mean[4]) / std[4];
        resc[(tx*(clos-3)+ty)*16+6] = (intensity[(tx+(5-x)%4)*clos+ty+(6-y)%4] - mean[5]) / std[5];
        resc[(tx*(clos-3)+ty)*16+7] = (intensity[(tx+(5-x)%4)*clos+ty+3-y] - mean[3])/ std[3];
        
        resc[(tx*(clos-3)+ty)*16+8] = (intensity[(tx+(6-x)%4)*clos+ty+(4-y)%4] - mean[6]) / std[6];
        resc[(tx*(clos-3)+ty)*16+9] = (intensity[(tx+(6-x)%4)*clos+ty+(5-y)%4] - mean[7]) / std[7]; 
        resc[(tx*(clos-3)+ty)*16+10] = (intensity[(tx+(6-x)%4)*clos+ty+(6-y)%4] - mean[8]) / std[8]; 
        resc[(tx*(clos-3)+ty)*16+11] = (intensity[(tx+(6-x)%4)*clos+ty+3-y] - mean[6]) / std[6];
        
        resc[(tx*(clos-3)+ty)*16+12] = (intensity[(tx+3-x)*clos+ty+(4-y)%4] - mean[0]) / std[0]; 
        resc[(tx*(clos-3)+ty)*16+13] = (intensity[(tx+3-x)*clos+ty+(5-y)%4] - mean[1]) / std[1]; 
        resc[(tx*(clos-3)+ty)*16+14] = (intensity[(tx+3-x)*clos+ty+(6-y)%4] - mean[2]) / std[2]; 
        resc[(tx*(clos-3)+ty)*16+15] = (intensity[(tx+3-x)*clos+ty+3-y] - mean[0]) / std[0];
        //if(tx == 0 && ty ==0){
...
data = data.reshape(-1)
resc = np.zeros(((row-3)*(cols-3),16)).astype(np.float32)
...
print('resc:', d_resc, d_resc.shape)